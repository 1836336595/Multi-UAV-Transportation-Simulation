...
%
%   3. **期望偏航角**：论文的 R0d 让负载第一轴指向速度方向，即偏航随路径转。
%      本实现保留这一参考生成方式；cfg.figureEight.lockYaw = true 时才将参考
%      yaw 固定为 0，便于先独立检查静态 yaw 跟踪。负载 yaw 反馈由控制器默认开启，
%      因此固定参考不是“关闭 yaw 控制”，而是只改变 R0d 的参考信号。
%
