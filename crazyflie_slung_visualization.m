...
subplot(2, 3, 6);
% ★ 同时画【期望 yaw】与【实际 yaw】，否则看不出"跟没跟上"。
%   期望 yaw 直接取仿真时记下的 sim.loadYawRefLog（= R0d 第一轴方位角 = 运动方向）。
%   ★ 两条曲线都必须 **unwrap（解卷绕）**：yaw 是角度，从 +179° 跳到 -179° 只是
%     跨过 ±180 割线、并非真的反向；不解卷绕就会看成"正负乱跳"。
